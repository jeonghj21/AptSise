...
insert into apt_sale_stats
        select region_key, 3, made_year, area_type, ym, 'Y', round(avg(price/(area/3.3)), 2) unit_price, round(avg(price), 2) price, count(*)
...
insert into apt_sale_stats
        select r.upper_region, 2, made_year, area_type, ym, danji_flag, round((sum(unit_price * cnt) / sum(cnt)), 2), round((sum(price * cnt) / sum(cnt)), 2), sum(cnt)
...
insert into apt_sale_stats
        select r.upper_region, 1, made_year, area_type, ym, danji_flag, round((sum(unit_price * cnt) / sum(cnt)), 2), round((sum(price * cnt) / sum(cnt)), 2), sum(cnt)
          from apt_sale_stats a, region_info r
         where a.region_key = r.region_key
           and a.level = 2
		   and a.danji_flag = 'Y'
...
insert into apt_sale_stats
        select '0000000000', 0, made_year, area_type, ym, danji_flag, round((sum(unit_price * cnt) / sum(cnt)), 2), round((sum(price * cnt) / sum(cnt)), 2), sum(cnt)
